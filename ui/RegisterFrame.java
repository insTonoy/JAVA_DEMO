...
public class RegisterFrame extends JFrame {
    ...
    public RegisterFrame() {
        ...
        patronIdLabel.setFont(new Font("Arial", Font.BOLD, 22));
        patronIdLabel.setBounds(250, 390, 150, 30);
        patronIdField = new JTextField(20);
        patronIdField.setFont(new Font("Arial", Font.BOLD, 20));
        patronIdField.setBounds(450, 390, 300, 35);
        mainPanel.add(patronIdLabel);
        mainPanel.add(patronIdField);
    ...
    private void handleRegistration() {
        ...
        if (username.isEmpty() || password.isEmpty() || name.isEmpty() || email.isEmpty() || patronId.isEmpty()) {
            JOptionPane.showMessageDialog(this, "All fields are required!", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        Patron existingPatron = PatronFileHandler.findPatronByUsername(username);
        if (existingPatron != null) {
            JOptionPane.showMessageDialog(this, "Username already exists. Please choose another one.", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }

        Patron newPatron= new Patron(username, password, name, email, patronId, bookBorrowedCount);
        PatronFileHandler.addPatron(newPatron);

        JOptionPane.showMessageDialog(this, "Patron registered successfully!", "Success", JOptionPane.INFORMATION_MESSAGE);
        clearFields();
    }
